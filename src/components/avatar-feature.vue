...
const props = defineProps<{ lang: "ru" | "en" }>();
const { lang } = toRefs(props);
const { showLoading, hideLoading } = useSystem();

const { sendMessage, messages, currentMessage, gtpIsStreaming, currentWord } =
  useOpenAI();
...
const onStopRecording = () => {
  ...
  setTimeout(() => {
    if (
      ...
      transcript.value.text !== userText.value
    ) {
...
<template>
  ...
  <div
    v-if="iceConnectionState === 'connected'"
    :class="{ 'opacity-0': state === 'stopped' }"
    class="flex space-x-2 mt-4 sm:flex-wrap sm:justify-center"
  >
    <UIButton
